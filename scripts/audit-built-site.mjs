...
if (publicFiles.has(`${base}/site-index.json`.replace(/\/{2,}/g, '/'))) {
  ...
  if (index.schemaVersion !== 1) failures.push('/site-index.json: schemaVersion must be 1');
...
if (siteConfig.templateMode && !/User-agent: \*\s+Disallow: \//m.test(robots)) {
  failures.push('/robots.txt: template mode must disallow all crawlers');
}
